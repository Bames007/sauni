...
//       e.preventDefault();
//       if (validate()) {
//         // If we have current secondary data that hasn't been added yet
//         if (
//           currentSecondary.examNumber &&
//           currentSecondary.schoolName &&
...
//       }
//     },
//     [formData, currentSecondary, validate, updateData, nextStep]
//   );
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      if (validate()) {
        ...
        console.log("=== ACADEMIC HISTORY FORM DATA ===");
        console.log("Final Data to be passed:", finalData);
        console.log("Form Data (formatted):", {
          primaryEducation: finalData.primaryEducation,
          secondarySchool: finalData.secondarySchool.map((sec) => ({
            examType: sec.examType,
            examNumber: sec.examNumber,
            schoolName: sec.schoolName,
            schoolType: sec.schoolType,
            completionYear: sec.completionYear,
            sitting: sec.sitting,
            grades: sec.grades,
          })),
          tertiaryEducation: finalData.tertiaryEducation?.map((tertiary) => ({
            institution: tertiary.institution,
            degree: tertiary.degree,
            fieldOfStudy: tertiary.fieldOfStudy,
            startYear: tertiary.startYear,
            endYear: tertiary.endYear,
            isCompleted: tertiary.isCompleted,
          })),
          qualifications: finalData.qualifications?.map((qual) => ({
            name: qual.name,
        ...
        console.log("=== END ACADEMIC HISTORY ===");

        updateData(finalData);
        nextStep();
      } else {
        console.log("=== ACADEMIC HISTORY FORM VALIDATION FAILED ===");
